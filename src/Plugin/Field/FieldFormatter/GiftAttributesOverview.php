...
/**
 * Plugin implementation of the 'commerce_gift_attributes_overview' formatter.
 *
 * @FieldFormatter(
 *   id = "commerce_gift_attributes_overview",
 *   label = @Translation("Gift attributes overview"),
 *   field_types = {
 *     "entity_reference",
 *   },
 * )
 */
class GiftAttributesOverview extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a new GiftAttributesOverview object.
   *
   ...
   *   The renderer.
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, EntityTypeManagerInterface $entity_type_manager, EntityDisplayRepositoryInterface $entity_display_repository, GiftAttributeFieldManagerInterface $attribute_field_manager, RendererInterface $renderer) {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $gift_type_storage = $this->entityTypeManager->getStorage('commerce_gift_type');
    $attribute_storage = $this->entityTypeManager->getStorage('commerce_gift_attribute');
    /** @var \Drupal\commerce_gift\Entity\GiftTypeInterface $gift_bundle */
    $gift_bundle = $gift_type_storage->load($this->fieldDefinition->getTargetBundle());
    $attribute_map = $this->attributeFieldManager->getFieldMap($gift_bundle->getVariationTypeId());
    $used_attributes = [];
    ...
    $view_modes = $this->entityDisplayRepository->getViewModes('commerce_gift_attribute_value');
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    $attribute_storage = $this->entityTypeManager->getStorage('commerce_gift_attribute');
    $elements = [];
    /** @var \Drupal\commerce_gift\Entity\GiftAttributeInterface[] $attributes */
    $attributes = $attribute_storage->loadMultiple(array_filter($this->getSetting('attributes')));
   ...
   *   The render array.
   */
  protected function getAttributeItemList(FieldItemListInterface $variation_items, GiftAttributeInterface $attribute) {
    $build = [
      '#theme' => 'item_list',
    ...
    $view_builder = $this->entityTypeManager->getViewBuilder('commerce_gift_attribute_value');
    /** @var \Drupal\Core\Field\Plugin\Field\FieldType\EntityReferenceItem $variation */
    foreach ($variation_items as $variation) {
      /** @var \Drupal\commerce_gift\Entity\GiftAttributeValueInterface $attribute_value */
  ...
  public static function isApplicable(FieldDefinitionInterface $field_definition) {
    ...
    return $entity_type == 'commerce_gift' && $field_name == 'variations';
  }
