...
class GiftAttributeForm extends BundleEntityFormBase {
  ...
  /**
   * Constructs a new GiftAttributeForm object.
   *
   * @param \Drupal\commerce_gift\GiftAttributeFieldManagerInterface $attribute_field_manager
   *   The attribute field manager.
   */
  public function __construct(GiftAttributeFieldManagerInterface $attribute_field_manager) {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\commerce_gift\Entity\GiftAttribute::load',
      ],
    ...
    $form['elementType'] = [
      ...
      '#options' => [
        ...
        'commerce_gift_rendered_attribute' => $this->t('Rendered attribute'),
      ],
      '#default_value' => $attribute->getElementType(),
    ];

    // Allow the attribute to be assigned to a gift variaton type.
    ...
    $form['variation_types'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('Gift variation types'),
    ];
    $attribute_field_map = $this->attributeFieldManager->getFieldMap();
    $variation_type_storage = $this->entityTypeManager->getStorage('commerce_gift_variation_type');
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if (!$attribute->isNew()) {
        ...
        $enabled = $translation_manager->isEnabled('commerce_gift_attribute_value', $attribute->id());
      }
  ...
  public function buildValuesForm(array $form, FormStateInterface $form_state) {
    ...
    $wrapper_id = Html::getUniqueId('gift-attribute-values-ajax-wrapper');
    $form['values'] = [
      '#type' => 'table',
      '#header' => [
      ...
      '#tabledrag' => [
        [
          ...
          'group' => 'gift-attribute-value-order-weight',
        ],
      ],
      '#weight' => 5,
    ...
    foreach ($value_map as $index => $id) {
      ...
      $value_form['weight'] = [
        ...
        '#attributes' => [
          'class' => ['gift-attribute-value-order-weight'],
        ],
      ];
    ...
    $access_handler = $this->entityTypeManager->getAccessControlHandler('commerce_gift_attribute_value');
  ...
  protected function saveValues(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($delete_queue)) {
      $value_storage = $this->entityTypeManager->getStorage('commerce_gift_attribute_value');
    ...
    foreach ($form_state->getValue(['values']) as $index => $value_data) {
      /** @var \Drupal\commerce_gift\Entity\GiftAttributeValueInterface $value */
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if ($translation_manager->isEnabled('commerce_gift_attribute_value', $this->entity->id()) != $enabled) {
        $translation_manager->setEnabled('commerce_gift_attribute_value', $this->entity->id(), $enabled);
    ...
    if ($status == SAVED_NEW) {
      drupal_set_message($this->t('Created the %label gift attribute.', ['%label' => $this->entity->label()]));
      // Send the user to the edit form to create the attribute values.
      $form_state->setRedirectUrl($this->entity->toUrl('edit-form'));
    }
    else {
      $this->saveValues($form, $form_state);
      drupal_set_message($this->t('Updated the %label gift attribute.', ['%label' => $this->entity->label()]));
