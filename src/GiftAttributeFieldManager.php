...
/**
 * Default implementation of the GiftAttributeFieldManagerInterface.
 */
class GiftAttributeFieldManager implements GiftAttributeFieldManagerInterface {
  ...
  /**
   * Constructs a new GiftAttributeFieldManager object.
   *
  ...
  public function getFieldDefinitions($variation_type_id) {
    if (!isset($this->fieldDefinitions[$variation_type_id])) {
      ...
      $definitions = array_filter($definitions, function ($definition) {
        ...
        return $field_type == 'entity_reference' && $target_type == 'commerce_gift_attribute_value';
      });
  ...
  public function getFieldMap($variation_type_id = NULL) {
    if (!isset($this->fieldMap)) {
      if ($cached_map = $this->cache->get('commerce_gift.attribute_field_map')) {
        $this->fieldMap = $cached_map->data;
      }
      else {
        $this->fieldMap = $this->buildFieldMap();
        $this->cache->set('commerce_gift.attribute_field_map', $this->fieldMap);
      }
    }
  ...
  public function clearCaches() {
    ...
    $this->fieldMap = NULL;
    $this->cache->delete('commerce_gift.attribute_field_map');
  }
  ...
  protected function buildFieldMap() {
    $field_map = [];
    $bundle_info = $this->entityTypeBundleInfo->getBundleInfo('commerce_gift_variation');
    foreach (array_keys($bundle_info) as $bundle) {
      /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $form_display */
      $form_display = commerce_get_entity_display('commerce_gift_variation', $bundle, 'form');
  ...
  public function createField(GiftAttributeInterface $attribute, $variation_type_id) {
    ...
    $field_storage = FieldStorageConfig::loadByName('commerce_gift_variation', $field_name);
    $field = FieldConfig::loadByName('commerce_gift_variation', $variation_type_id, $field_name);
    if (empty($field_storage)) {
      $field_storage = FieldStorageConfig::create([
        ...
        'cardinality' => 1,
        'settings' => [
          'target_type' => 'commerce_gift_attribute_value',
        ],
        'translatable' => FALSE,
      ]);
      $field_storage->save();
    ...
    if (empty($field)) {
      ...
      /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $form_display */
      $form_display = commerce_get_entity_display('commerce_gift_variation', $variation_type_id, 'form');
  ...
  public function canDeleteField(GiftAttributeInterface $attribute, $variation_type_id) {
    ...
    $query = $this->queryFactory->get('commerce_gift_variation')
   ...
   *   The field name.
   */
  protected function buildFieldName(GiftAttributeInterface $attribute) {
